...
class DistributedWeightedSampler(torch.utils.data.Sampler):
    ...
    def calculate_weights(self, targets):
        ...
        class_sample_weight = {
            t.item():1./(targets == t).sum() for t in torch.unique(targets, sorted=True)}
        #weight = 1. / class_sample_count.double()
        # print(weight)
        # print(weight.size())
        # print(targets.size())
        samples_weight = torch.tensor([class_sample_weight[t.item()] for t in targets])
    ...
    def __iter__(self):
        ...
        targets = self.dataset.labellist
