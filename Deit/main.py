...
from engine import train_one_epoch, evaluate, evaluate_submit
from losses import DistillationLoss
from samplers import RASampler, DistributedWeightedSampler
import models
import utils
...
def get_args_parser():
    ...
    # * Finetuning params
    # parser.add_argument('--finetune', default='/youtu-reid/jiaxzhuang/pretrained/deit_small_distilled_patch16_224-649709d9.pth', help='finetune from checkpoint')
    ...
    parser.add_argument('--fix-backbone', action='store_true', help='whether to fix backbone')
...
def main(args):
    ...
    if args.trainval:
        dataset_train, args.nb_classes = build_dataset(is_train='trainval', args=args)
    else:
        dataset_train, args.nb_classes = build_dataset(is_train='train', args=args)
    dataset_val, _ = build_dataset(is_train='val', args=args)

    if True:  # args.distributed:
        num_tasks = utils.get_world_size()
        global_rank = utils.get_rank()
        if args.repeated_aug and not args.weight_sampler:
            ...
            global weights
            print('Using weight sampler for traiing fc, ')
    ...
    if args.finetune:
        ...
        for k in ['head.weight', 'head.bias', 'head_dist.weight', 'head_dist.bias']:
            ...
            if k in checkpoint_model and args.init_fc:
                print(f"Removing key {k} from pretrained checkpoint")
                del checkpoint_model[k]
    ...
    if args.eval:
        test_stats = evaluate_submit(data_loader_val, model, device, args)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['mca']:.1f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # if args.distributed and not args.weight_sampler:
        if args.distributed:
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['mca']:.1f}%")
        max_accuracy = max(max_accuracy, test_stats["mca"])
        print(f'Max MCA: {max_accuracy:.2f}%')
...
def get_last_checkpoint(checkpoint_dir):
    all_ckpt = glob.glob(os.path.join(checkpoint_dir, 'checkpoint.pth'))
    if all_ckpt:
